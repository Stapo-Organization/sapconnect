...
## الحالة (2026-09-17)
- Internal testing: 1.0.0 (36) مكتمل، القوائم ar/en-US + الأيقونة + الرسم الترويجي + 5 لقطات مرفوعة عبر `play.rb`؛ `edits:validate` يمرّ.
...
- بعد اكتمال App content: الإصدار للإنتاج من الطرفية —
  `edits` → `tracks/production` بنفس versionCode (36) + `status: completed` → `${E}:commit`.
